const map1_viz = (loadData) => {
    ...
    var map1_svg = d3.select("#maps_map1")
        .append("svg")
        .attr("id", "tab1map")
        .attr("width", w_perc)
        .attr("height", h_perc)
    ...
    d3.select("#cases_vax_switch_map1")
        .on("change", function () {
            if (document.getElementById("cases_vax_switch_map1").checked)
                var_type = "vax";
            else
                var_type = "cases";

            data = loadData[1].filter(e => e.date == interesting_date);
            plot_data(topo, data, var_type);
        })

    // event listener for slider changes
    d3.select("#date_slider_map1")
        .on("change", function () {
            ...
            var cv_switch = d3.select("#cases_vax_switch_map1");
            if (offset < 9) {
    ...
    function plot_data(topo, data, variable_type) {
        const vax = "vax";
        const cases = "cases";
        ...
        map1_svg.selectAll(".Country")
            .on("mousemove", function (event, d) {
                ...
                [mouse_x, mouse_y] = d3.pointer(event, map1_svg);
                ...
                // if we have a number then print it with 2 decimal positions
